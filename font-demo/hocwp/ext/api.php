...
function hocwp_api_allow_meta_query( $valid_vars ) {
	$valid_vars = array_merge( $valid_vars, array( 'meta_key', 'meta_value', 'meta_query' ) );

	return $valid_vars;
}

add_filter( 'rest_query_vars', 'hocwp_api_allow_meta_query' );

function hocwp_api_get_by_meta( $meta_query, $rest_base, $version = 'v2', $server = HOCWP_API_SERVER ) {
	$fields     = array(
