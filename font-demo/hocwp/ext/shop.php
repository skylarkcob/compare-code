		...
		if ( has_post_thumbnail() ) {
			...
			if ( ! empty( $image_url ) ) {
				...
				$atts  = array(
					...
					'title'    => esc_attr( $props['caption'] ),
					'data-rel' => sprintf( 'prettyPhoto%s', $gallery )
				);
				$atts  = hocwp_get_attr( 'entry-thumbnail', 'single-thumbnail', $atts );
				$html  = sprintf( '<a %1$s>%2$s</a>', $atts, $image );
				$html  = apply_filters( 'hocwp_wc_single_product_image_html', $html, $post->ID );
			}
		}
	}
	unset( $thumbnail_zoom );
