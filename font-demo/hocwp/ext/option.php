...
function hocwp_plugin_option_page_sidebar() {
	global $hocwp;
	if ( hocwp_array_has_value( $hocwp->plugin->option->sidebar_tabs ) ) {
		$current_page = hocwp_get_current_admin_page();
		?>
		<ul class="list-tabs">
			<?php foreach ( $hocwp->plugin->option->sidebar_tabs as $key => $value ) : ?>
				<?php
...
function hocwp_theme_option_sidebar_tab() {
	global $hocwp;
	$tabs = $hocwp->theme->option->sidebar_tabs;
