...
function hocwp_slider_meta_box_field( $post_type, $post ) {
	...
	if ( 'hocwp_slider' == $post_type ) {
		...
		$args = array(
			'id'             => 'autoplay',
			'label'          => __( 'Make slider autoplay.', 'hocwp-theme' ),
			'field_callback' => 'hocwp_field_checkbox',
			'default'        => 0
		);
		$meta->add_field( $args );
...
function hocwp_slider_on_save_slider_info( $post_id ) {
	...
	if ( 'hocwp_slider' == $slider->post_type ) {
		...
		$value = hocwp_checkbox_post_data_value( $_POST, 'autoplay' );
		update_post_meta( $post_id, 'autoplay', $value );
	}
}
...
function hocwp_slider_html( $args = array() ) {
	...
	if ( hocwp_is_post( $slider ) ) {
		...
		if ( ! empty( $order ) ) {
			...
			$autoplay = hocwp_get_post_meta( 'autoplay', $slider->ID );
			if ( ! $autoplay ) {
				$autoplay = hocwp_get_value_by_key( $args, 'autoplay' );
			}
			$list_class = 'list-unstyled list-items slickslide list-inline';
			if ( $autoplay ) {
				hocwp_add_string_with_space_before( $list_class, 'autoplay' );
			}
			echo '<div class="' . $slider_class . '">';
			echo '<ul class="' . $list_class . '"' . $atts . '>';
			$list_paging = '';
			...
			foreach ( $order as $item_id ) {
				...
				if ( hocwp_array_has_value( $item ) ) {
					...
					if ( ! empty( $description ) ) {
						...
						$div->set_class( 'description' );
						$div->set_text( $description );
						$li_html .= $div->build();
					}
					$li->set_text( $li_html );
					$li->output();
