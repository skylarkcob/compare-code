...
require( dirname( __FILE__ ) . '/php-data.php' );

function hocwp_require_if_file_exists( $file, $require_once = false ) {
	if ( file_exists( $file ) ) {
		if ( $require_once ) {
			require_once( $file );
		} else {
...
function hocwp_include_if_file_exists( $file, $include_once = false ) {
	if ( file_exists( $file ) ) {
		if ( $include_once ) {
			include_once( $file );
		} else {
...
function hocwp_add_attribute( &$attributes, $attribute_name = '', $value ) {
	...
	if ( is_array( $attributes ) ) {
		$atts = isset( $attributes[ $attribute_name ] ) ? $attributes[ $attribute_name ] : '';
		if ( is_array( $atts ) ) {
			$atts[] = $value;
		} else {
			if ( ! hocwp_string_contain( $atts, $value ) ) {
				hocwp_add_string_with_space_before( $atts, $value );
			}
		}
		$attributes[ $attribute_name ] = $atts;
	} else {
		$attributes = array(
