...
class HOCWP_HTML {
	...
	public function add_attribute( $attribute_name, $value ) {
		$rels = $this->get_attribute( $attribute_name );
		if ( is_array( $rels ) ) {
			if ( ! hocwp_in_array( $value, $rels ) ) {
				$rels[] = $value;
				$this->set_attribute( $attribute_name, $rels );
			}
		} else {
		...
		if ( 'a' == strtolower( $this->get_name() ) ) {
			...
			if ( ! empty( $href ) ) {
				if ( ! hocwp_is_site_domain( $href ) && ! hocwp_string_contain( $href, 'javascript' ) ) {
					$this->add_attribute( 'rel', 'nofollow' );
					$this->add_attribute( 'rel', 'external' );
