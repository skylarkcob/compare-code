...
function hocwp_sanitize_array( $arr, $deprecated = '', $dep = '' ) {
...
function hocwp_sanitize_media_value( $value ) {
	...
	$id  = absint( $id );
	if ( ! empty( $url ) && ! hocwp_id_number_valid( $id ) ) {
	...
	$result = array(
		'id'          => $id,
		'url'         => $url,
		'path'        => hocwp_get_media_file_path( $id ),
		'type_icon'   => $icon,
...
function hocwp_get_media_info( $id ) {
	$data          = hocwp_sanitize_media_value( array( 'id' => $id ) );
...
function hocwp_get_attr( $slug, $context = '', $atts = array(), $attr = '' ) {
	if ( function_exists( 'hybrid_get_attr' ) ) {
		$atts = wp_parse_args( $atts, apply_filters( 'hybrid_attr_' . $slug, array(), $context ) );
	}
	if ( empty( $atts ) ) {
		$atts['class'] = $slug . ' ' . $context;
	} else {
		hocwp_add_attribute( $atts, 'class', $context );
	}
	$atts = apply_filters( 'hocwp_get_attr', $atts, $slug, $context );
	$atts = apply_filters( 'hocwp_get_attr_' . $slug, $atts, $context );
	$id   = isset( $atts['id'] ) ? $atts['id'] : '';
	if ( ! empty( $id ) ) {
		hocwp_add_attribute( $atts, 'class', $id );
	}
	$out = '';
	...
	if ( ! empty( $attr ) ) {
		hocwp_add_string_with_space_before( $out, $attr );
	}

	return trim( $out );
}

function hocwp_attr( $slug, $context = '', $atts = array(), $attr = '' ) {
	echo hocwp_get_attr( $slug, $context, $atts, $attr );
}