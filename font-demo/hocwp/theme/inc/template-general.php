...
function hocwp_addthis_script( $args = array() ) {
	...
	<script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pubid=<?php echo $id; ?>"
	        async="async"></script>
	<?php
}
...
function hocwp_addthis_toolbox( $args = array() ) {
	...
	<div class="<?php echo $class; ?>" data-url="<?php echo $url; ?>"
	     data-title="<?php echo hocwp_wpseo_get_post_title( $post_id ); ?>"></div>
	<?php
}
...
function hocwp_theme_the_logo() {
	...
	if ( empty( $logo_url ) ) {
		$title_text = $blog_name;
	} else {
		$title_text = '<img alt="' . $desc . '" src="' . $logo_url . '">';
	...
	<div id="branding" class="site-branding">
		<?php
		$format     = '<a class="%1$s" title="%2$s" href="%3$s" rel="home">%4$s</a>';
		$title_text = sprintf( $format, $logo_class, $blog_name, esc_url( home_url( '/' ) ), $title_text );
		if ( is_front_page() && is_home() ) {
			$tag = 'h1';
		} else {
			$tag = 'p';
		}
		$site_title = sprintf( '<%1$s ' . hocwp_get_attr( 'site-title' ) . '>%2$s</%1$s>', $tag, $title_text );
		echo $site_title;
		if ( display_header_text() ) {
			if ( ! empty( $desc ) ) {
				$desc = sprintf( '<h2 %1$s>%2$s</h2>', hocwp_get_attr( 'site-description' ), $desc );
				echo $desc;
			}
		}
		do_action( 'hocwp_theme_logo' )
		?>
	</div><!-- .site-branding -->
	<?php
}
...
function hocwp_theme_the_menu( $args = array() ) {
	...
	?>
	<nav <?php hocwp_attr( 'menu', $theme_location, $atts ); ?>>
		<?php
		$menu_args = array(
...
function hocwp_theme_site_main_before( $class = '', $outer_class = '' ) {
	...
	<div id="primary" class="<?php echo $outer_class; ?>">
		<main <?php hocwp_attr( 'content', '', $atts ); ?>>
			<?php
			}
...
function hocwp_the_date() {
	...
	<time datetime="<?php the_time( 'c' ); ?>" itemprop="datePublished"
	      class="entry-time published date post-date"><?php echo get_the_time( hocwp_get_date_format(), $post ); ?></time>
	<?php
}
		...
		if ( ! empty( $format ) ) {
			...
			if ( $show_updated ) {
				...
				<time datetime="<?php the_modified_time( 'c' ); ?>" itemprop="dateModified"
				      class="entry-modified-time date modified post-date updated"><?php echo get_the_modified_time( hocwp_get_date_format() ); ?></time>
				<?php
			}
			if ( $show_term ) {
				$meta_term_args = $args;
