...
function hocwp_load_addthis_script() {
	...
	if ( $use ) {
		$page = apply_filters( 'hocwp_use_addthis_on_page', $use );
		$home = apply_filters( 'hocwp_use_addthis_on_home', $use );
...
function hocwp_setup_theme_pre_get_posts( WP_Query $query ) {
	if ( $query->is_main_query() ) {
		if ( is_home() ) {
			if ( ! function_exists( 'hocwp_option_home_setting' ) && function_exists( 'Pixelify' ) ) {
				Pixelify()->load_plugin_theme_file( '/font-demo/hocwp/theme/inc/options.php', '/inc/options/setting-theme-home.php' );
			}

			if ( ! function_exists( 'hocwp_option_home_setting' ) ) {
				require_once( get_template_directory() . '/inc/options/setting-theme-home.php' );
			}
