...
if ( ! function_exists( 'hocwp_get_wpseo_social_value' ) && function_exists( 'Pixelify' ) ) {
	require_once Pixelify()->get_basedir() . '/font-demo/hocwp/ext/seo.php';
}
...
function hocwp_addthis_toolbox( $args = array() ) {
	...
	$button_compact = hocwp_get_value_by_key( $args, 'button_compact' );
	if ( $button_compact ) {
		$type = 'addthis_button_compact';
		hocwp_add_string_with_space_before( $class, $type );
		printf( '<a addthis:url="%1$s" addthis:title="%2$s" class="%3$s" href="#"><span>Chia sẻ</span></a>', $url, $title, $class );

		return;
	}
	if ( empty( $type ) ) {
		$type = 'addthis_native_toolbox';
	}
	hocwp_add_string_with_space_before( $class, $type );
	?>
	<!-- Go to www.addthis.com/dashboard to customize your tools -->
