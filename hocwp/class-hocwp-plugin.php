...
if ( ! class_exists( 'Pixelify_Function' ) ) {
	class Pixelify_Function {
...
if ( ! function_exists( 'HP' ) ) {
	function HP() {
		return Pixelify_Function::get_instance();
	}
}
...
if ( ! class_exists( 'HOCWP_Plugin_Core' ) ) {
	abstract class HOCWP_Plugin_Core {
		...
		public function get_option_post( $option_name ) {
			$id = $this->get_option( $option_name );
